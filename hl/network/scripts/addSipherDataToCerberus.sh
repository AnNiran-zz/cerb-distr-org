...
for osinstance in $(echo "${osinstances}" | jq -r '.[] | @base64'); do
		...
		# check if configuration file is present on remote host machine
		sshpass -p "${!osPasswordVar}" ssh ${!osUsernameVar}@${!osHostVar} "test -e ${!osPathVar}hl/network/external-orgs/sipher-data.json"
		result=$?
		echo $result

		if [ $result -ne 0 ]; then
			sshpass -p "${!osPasswordVar}" scp sipher-config/$SIPHER_CONFIG_FILE ${!osUsernameVar}@${!osHostVar}:${!osPathVar}hl/network/external-orgs

			if [ "$?" -ne 0 ]; then
				echo "Cannot copy Sipher configuration file on ${osHostVar} remote machine"
				exit 1
			fi

			echo
			echo "Sipher configuration file copied successfully to ${!osHostVar} machine"
			echo

		else
			echo
			echo "Sipher configuration file is already present on ${!osHostVar} remote machine"
			echo
		fi

		# start remote script
		if [ "${dataType}" == "env" ]; then
			sshpass -p "${!osPasswordVar}" ssh ${!osUsernameVar}@${!osHostVar} "cd ${!osPathVar}hl/network && bash scripts/addOrgEnvData.sh external-orgs/sipher-data.json"
			result=$?
			echo $result
