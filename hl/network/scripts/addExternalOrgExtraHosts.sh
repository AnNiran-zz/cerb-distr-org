...
function addCliExtraHost() {
        ...
        if [[ " ${extraHostsParsed[*]} " == *"$newHostContainer"* ]]; then
                echo "$newHost already in list of extra hosts for icontainer"
        else
                yq write --inplace sipher-$container.yaml services[cli."${container}".sipher.cerberus.net].extra_hosts[+] $newHost
                echo "added ${extraHost} in list of extra_hosts for cli.$container"
        fi
}
        ...
        if [ "$cliHosts" == null ]; then
                ...
                for orgContainer in $(echo "${orgContainers}" | jq -r '. | @base64'); do
                        _jq(){
                        	...
                        	addCliExtraHost $cliHosts $container $orgContainerValueStripped $extraHost
                        }
                        echo $(_jq '.container' '.host')
                done
        fi
